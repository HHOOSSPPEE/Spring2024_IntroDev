...
var keyAttack = keyboard_check(vk_enter);
var keyInteract = keyboard_check(vk_shift);

//moving
if (keyLeft) x -= player_speed;
if (keyRight) x += player_speed;
if (keyUp && !place_meeting(x, y + 17, tmWalls)) y -= player_speed;
...
//attack from enemy
if (place_meeting(x, y, obj_all_enemy) && !invincible && sprite_index != spr_p1_attack && !isDead)
{
	player_health--;
	invincible = true;
	global.p1_health = player_health;
}
...
if (place_meeting(x, y, potion) && keyInteract && isClear)
{
	if (room == rm_main)
	{
		global.p1_curse1 = true;
	}
	else if (room = rm_room2)
	{
		global.p1_curse2 = true;
	}
	else if (room == rm_room3)
	{
		global.p1_curse3 = true;
	}
