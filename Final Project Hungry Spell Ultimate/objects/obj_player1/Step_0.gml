...
// 检测并处理各种食物的碰撞
handle_food_collision(obj_orange, 45);
handle_food_collision(obj_melon, 70);
handle_food_collision(obj_food, 20);
...
RoomGameOver=room;
if (global.shield == 0) {
    room_goto(RoomGameOver); // Replace with actual game over room
    //global.shield = 3; // Reset shield for next life
}

// Check for running out of value (health, energy, etc.)
if (global.value == 0) {
    room_goto(RoomGameOver); // Replace with actual game over room
}

// Check for falling below the stage
if (y > 740) {
    room_goto(RoomGameOver); // Replace with actual game over room
}


if(x>11500&&room=Room1){ 
	if(isGrounded>0){
room_goto(rm_2)
}
